...
- **Standard-Zugangsdaten nach dem ersten Flash:** Benutzername `admin`, Passwort aus `secrets.h` (`CONFIG_PASSWORD`) bei selbst kompilierter Firmware bzw. **`admin123`** bei den fertigen `.bin`-Dateien aus den [Releases](https://github.com/mallewski/TaupunktLueftung/releases). Beides lässt sich anschließend im Webinterface unter Einstellungen → Zugang (Login) ändern. **Solange noch admin/admin123 aktiv ist, zeigt das Dashboard automatisch einen Warnhinweis an** – dieser verschwindet erst, sobald ein eigenes Passwort vergeben wurde.
